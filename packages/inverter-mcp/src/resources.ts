...
function controlContractMarkdown(): string {
  ...
  return [
    ...
    "",
    "## Season profiles (`set_season_profile`)",
    "",
    "Three named presets flip the pair of priorities above in one action, so the house does not",
    "sit half-switched between two seasons:",
    "",
    `  - \`winter\` — output ${OUTPUT_SOURCE_PRIORITY[3]}, charging ${CHARGER_SOURCE_PRIORITY[0]}.`,
    "    The grid powers the house and keeps the battery full as a reserve.",
    `  - \`night\` — for a two-rate meter. 23:00–07:00 (cheap rate): output ${OUTPUT_SOURCE_PRIORITY[3]},`,
    `    charging ${CHARGER_SOURCE_PRIORITY[0]}. Day: output ${OUTPUT_SOURCE_PRIORITY[2]}, charging`,
    `    ${CHARGER_SOURCE_PRIORITY[3]} — the battery runs the house down to socBackToUtility (register 341).`,
    "    If the charge drops to 30% during the day the grid tops it up to 50%. The server switches",
    "    registers 301/331 on schedule by itself (logged as `schedule:night-tariff`) until another",
    "    profile or a manual change of either priority turns it off. `get_snapshot` shows it in",
    "    `nightTariff`.",
    `  - \`summer\` — output ${OUTPUT_SOURCE_PRIORITY[2]}, charging ${CHARGER_SOURCE_PRIORITY[1]}.`,
    "    PV and the battery power the house; the grid steps in only on overload or heavy cloud.",
    "",
    "A step whose register already holds the wanted value is skipped. One released lock covers",
    "the whole profile, and the lock re-engages once at the end.",
    "",
    "## Safety rules",
    "",
    "- Preview first (`set_control` or `set_season_profile` with `preview: true`).",
    "- Change one parameter at a time and verify with `get_snapshot` — a season profile is the",
    "  one sanctioned exception: its two priorities belong together and are meant to be applied",
    "  in one action.",
