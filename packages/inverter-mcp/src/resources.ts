...
function controlContractMarkdown(): string {
  ...
  return [
    ...
    `  - \`night\` — output ${OUTPUT_SOURCE_PRIORITY[3]}; charging ${CHARGER_SOURCE_PRIORITY[0]} from 23:00`,
    `    to 07:00 (the cheap rate of a two-rate meter) and ${CHARGER_SOURCE_PRIORITY[3]} during the day.`,
    "    If the charge drops to 30% during the day the grid tops it up to 50%. The server switches",
    "    register 331 on schedule by itself (logged as `schedule:night-tariff`) until another",
