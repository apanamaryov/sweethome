...
describe("liveArgs", () => {
  ...
  it("звук идёт вместе с видео и ничего не перекодируется", () => {
    // Звук больше не выбрасывается: у двух камер из трёх он есть, и зритель
    // может его включить. Что реально оказалось в потоке, решает не эта команда,
    // а заголовок (см. live/session.ts) — поэтому здесь просто нет "-an".
