...
```bash
ssh -i ~/.ssh/personal_190726 pi@192.168.1.112 \
  'ffmpeg -hide_banner -version | head -1; ffmpeg -hide_banner -protocols 2>/dev/null | grep -c rtsp; ffmpeg -hide_banner -muxers 2>/dev/null | grep -cE " hls| mp4"'
```

Ожидается: версия ≥ 5, `rtsp` найден (счётчик ≥ 1), muxers `hls` и `mp4` найдены (счётчик ≥ 2). Если rtsp отсутствует — остановиться: пакет собран без нужного, задача не выполнена.
