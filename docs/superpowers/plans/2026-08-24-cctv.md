...
> **Правка по итогам реализации (2026-08-25).** Тест из шага 1 подставляет только `URL.createObjectURL`, а очистка в компоненте зовёт ещё и `URL.revokeObjectURL`, которого в jsdom нет, — падали все четыре теста (RTL размонтирует компонент после каждого). Правильно: обернуть `revokeObjectURL` в `try/catch` (очистка при размонтировании не имеет права падать — тот же довод, что и у соседнего `endOfStream()`) **и** подставить `revokeObjectURL` в тесте, иначе тест зеленеет благодаря обёртке, а не проверяет поведение.
