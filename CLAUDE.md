...
Работает на Raspberry Pi (обычно `pi@192.168.1.112`).
...
- Деплой на дев-Pi — можно; учитывай, что `deploy.sh` пересобирает всё локально, заливает артефакты и **рестартует живой systemd-сервис** на Pi. Параметры: `PI_HOST=pi@… SSH_KEY=~/.ssh/… ./deploy.sh` (рабочий ключ — `~/.ssh/personal_190726`).
...
- USB-B порт инвертора занят BMS батареи — путь только через RS232.
